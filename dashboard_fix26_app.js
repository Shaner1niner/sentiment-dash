...
// BEGIN phase_seta_history_trace_patch_v12
(function phase_seta_history_trace_patch_v12(){
  if (window.__phase_seta_history_trace_patch_v12) return;
  window.__phase_seta_history_trace_patch_v12 = true;
  ...
  function scoreFromCell(cell){
    ...
    let m = txt.match(/SETA\s+Score\s*(-?\d+(?:\.\d+)?)/i);
    if (!m) m = txt.match(/Summary\s+Score\s*(-?\d+(?:\.\d+)?)/i);
    if (!m) m = txt.match(/(-?\d+(?:\.\d+)?)/);
    return m ? toNum(m[1]) : null;
  }

  function fmtDelta(scores, i, n){
    const cur = scores[i];
    const prev = i >= n ? scores[i - n] : null;
    if (cur === null || prev === null) return n + "d n/a";
    const d = cur - prev;
    const sign = d > 0 ? "+" : "";
    return n + "d " + sign + d.toFixed(1);
  }

  function fmtPctl(scores, i, n){
    const cur = scores[i];
    if (cur === null) return n + "d Pctl n/a";
    const start = Math.max(0, i - n + 1);
    const vals = scores.slice(start, i + 1).filter(v => v !== null && Number.isFinite(v));
    if (vals.length < 5) return n + "d Pctl n/a";
    const belowEq = vals.filter(v => v <= cur).length;
    const p = Math.round((belowEq / vals.length) * 100);
    return n + "d Pctl " + p;
  }

  function isActivePriceTooltip(trace){
    const ht = String(trace && trace.hovertemplate || "");
    return ht.includes("<b>SETA</b> %{customdata[0]}") &&
           ht.includes("Ribbon %{customdata[2]}") &&
           ht.includes("RSI %{customdata[4]}") &&
           ht.includes("Stoch %{customdata[5]}");
  }

  function patchTrace(trace){
    if (!trace || trace.__setaHistoryTracePatchV12) return false;
    if (!Array.isArray(trace.customdata) || !trace.customdata.length) return false;
    if (!isActivePriceTooltip(trace)) return false;

    const rows = trace.customdata.map(row => Array.isArray(row) ? row.slice() : [row]);
    const scores = rows.map(row => scoreFromCell(row[0]));
    const baseLen = Math.max(6, ...rows.map(row => row.length));
    const historyIdx = baseLen;

    trace.customdata = rows.map((row, i) => {
      while (row.length < baseLen) row.push("");
      row.push(fmtDelta(scores, i, 5) + " · " + fmtDelta(scores, i, 20) + " · " + fmtPctl(scores, i, 90));
      return row;
    });

    const oldTemplate = String(trace.hovertemplate || "");
    const needle = "<b>SETA</b> %{customdata[0]}<br>";
    const insert = "<b>SETA</b> %{customdata[0]}<br>SETA history: %{customdata[" + historyIdx + "]}<br>";
    trace.hovertemplate = oldTemplate.includes(needle)
      ? oldTemplate.replace(needle, insert)
      : oldTemplate.replace(/<extra><\/extra>$/, "<br>SETA history: %{customdata[" + historyIdx + "]}<extra></extra>");

    trace.__setaHistoryTracePatchV12 = true;
    return true;
  }
  ...
  function patchData(data){
    if (!Array.isArray(data)) return false;
    let changed = false;
    data.forEach(trace => { if (patchTrace(trace)) changed = true; });
    return changed;
  }
  ...
  function patchExistingPlots(){
    ...
    document.querySelectorAll(".js-plotly-plot").forEach(div => {
      let changed = false;
      if (Array.isArray(div.data) && patchData(div.data)) changed = true;
      if (Array.isArray(div._fullData) && patchData(div._fullData)) changed = true;
      if (changed && window.Plotly.redraw) {
        try { window.Plotly.redraw(div); } catch (err) { console.warn("SETA history trace redraw skipped:", err); }
      }
    });
  }

  function wrapPlotly(){
    if (!window.Plotly || window.__phase_seta_history_trace_patch_v12_plotly_wrapped) return;
    window.__phase_seta_history_trace_patch_v12_plotly_wrapped = true;
    ["newPlot", "react"].forEach(fn => {
      const original = window.Plotly[fn];
      if (typeof original !== "function") return;
      window.Plotly[fn] = function(){
        try { patchData(arguments[1]); } catch (err) { console.warn("SETA history trace pre-render patch skipped:", err); }
        return original.apply(this, arguments);
      };
    });
  }
  ...
  const timer = setInterval(() => {
    wrapPlotly();
    patchExistingPlots();
    if (window.Plotly && window.__phase_seta_history_trace_patch_v12_plotly_wrapped) clearInterval(timer);
  }, 250);
  setTimeout(() => clearInterval(timer), 10000);
  document.addEventListener("DOMContentLoaded", () => setTimeout(patchExistingPlots, 500));
})();
// END phase_seta_history_trace_patch_v12
