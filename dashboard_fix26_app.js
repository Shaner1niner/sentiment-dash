...
// BEGIN phase_market_tape_card_skeleton_v1
(function phase_market_tape_card_skeleton_v1(){
  if (window.__phase_market_tape_card_skeleton_v1) return;
  window.__phase_market_tape_card_skeleton_v1 = true;

  function installMarketTapeCardSkeletonStyle(){
    if (document.getElementById("phase_market_tape_card_skeleton_v1_style")) return;
    const style = document.createElement("style");
    style.id = "phase_market_tape_card_skeleton_v1_style";
    style.textContent = `
      /* Market Tape ranked card skeleton v1
         Goal: every ranked card uses the same visual rhythm:
         row 1 rank/asset + score, row 2 badges, row 3 setup title, row 4 reason.
         Extra space stays in the badge row / upper-right area instead of shifting the story block. */
      ...
      /* Row 1: rank/asset left, score right. Works with the existing first child wrapper. */
      .marketTapeCards > * > :first-child{
      ...
      /* Row 2: badge shelf. Keep it fixed-height so title/reason start at the same vertical point. */
      .marketTapeCards > * > :nth-child(2){
      ...
      /* Badge/pill hygiene inside ranked cards only. */
      .marketTapeCards > * > :nth-child(2) span,
      .marketTapeCards > * > :nth-child(2) .marketTapePill,
      .marketTapeCards > * > :nth-child(2) [class*="pill" i],
      .marketTapeCards > * > :nth-child(2) [class*="badge" i]{
      ...
      /* Row 3: setup title, anchored consistently below badges. */
      .marketTapeCards > * > :nth-child(3){
      ...
      /* Row 4+: reason text. Clamp so card height stays even while preserving scan value. */
      .marketTapeCards > * > :nth-child(n+4){
        overflow:hidden !important;
        line-height:1.18 !important;
      }

      .marketTapeCards > * > :nth-child(4){
        display:-webkit-box !important;
        -webkit-line-clamp:2 !important;
        -webkit-box-orient:vertical !important;
      }

      /* Keep the right-side score from drifting or wrapping. */
      .marketTapeCards > * > :first-child > :last-child{
  ...
  if (document.readyState === "loading"){
    document.addEventListener("DOMContentLoaded", installMarketTapeCardSkeletonStyle, {once:true});
  } else {
    installMarketTapeCardSkeletonStyle();
  }
})();
// END phase_market_tape_card_skeleton_v1
