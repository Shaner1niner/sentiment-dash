...
// BEGIN phase_seta_dropdown_contrast_v1
(function phase_seta_dropdown_contrast_v1(){
  if (window.__phase_seta_dropdown_contrast_v1) return;
  window.__phase_seta_dropdown_contrast_v1 = true;

  function injectStyle(){
    if (document.getElementById("phase_seta_dropdown_contrast_v1_style")) return;
    if (!document.head) return;
    const style = document.createElement("style");
    style.id = "phase_seta_dropdown_contrast_v1_style";
    style.textContent = `
      select,
      select option,
      select optgroup{
      ...
      select{
        color-scheme:dark;
        border-color:rgba(92,140,160,.45);
      }
      select:focus{
        outline:none;
  ...
  function styleOptions(){
    document.querySelectorAll("select").forEach(sel => {
      try{
        ...
        sel.querySelectorAll("option,optgroup").forEach(opt => {
          opt.style.backgroundColor = "#0b0f14";
          opt.style.color = "#e7eef3";
        });
      }catch(_){}
    });
  }
  ...
  function boot(){
    ...
    document.addEventListener("focusin", ev => {
      if (ev && ev.target && ev.target.tagName === "SELECT") styleOptions();
    });
    document.addEventListener("mousedown", ev => {
      if (ev && ev.target && ev.target.tagName === "SELECT") styleOptions();
    }, true);
    const obs = new MutationObserver(() => styleOptions());
...
// END phase_seta_dropdown_contrast_v1
