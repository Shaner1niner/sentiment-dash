...
// BEGIN phase_seta_render_debounce_v1
(function phase_seta_render_debounce_v1(){
  if (window.__phase_seta_render_debounce_v1) return;
  window.__phase_seta_render_debounce_v1 = true;
  ...
  window.SETA_RENDER_DEBOUNCE = STATE;
  ...
  function markControlActivity(){
    STATE.lastControlAt = now();
  }

  function isDashboardControlTarget(target){
    if (!target || !target.closest) return false;
    if (target.closest(".setaCustomSelectWrap")) return true;
  ...
  function getPlotKey(gd){
    if (gd && typeof gd === "object") return {store:pendingByElement, key:gd};
    if (typeof gd === "string") return {store:pendingByKey, key:gd};
    return {store:pendingByKey, key:"__default_plot__"};
  }

  function targetLabel(gd){
    if (typeof gd === "string") return gd;
    if (gd && gd.id) return "#" + gd.id;
    if (gd && gd.className) return "." + String(gd.className).split(/\s+/)[0];
    return "plot";
  }

  function shouldDebounce(){
    if (!STATE.enabled) return false;
    return (now() - STATE.lastControlAt) <= STATE.activeWindowMs;
  }

  function flushScheduled(original, ctx, pending){
    const args = pending.args;
    const resolvers = pending.resolvers.slice();
    pending.resolvers.length = 0;
  ...
  function schedulePlot(original, ctx, argsLike, fnName){
    const args = Array.from(argsLike);
    const gd = args[0];
    const {store, key} = getPlotKey(gd);
    let pending = store.get(key);

    if (!pending){
      pending = {timer:null, args:null, resolvers:[], ctx:null};
    ...
    STATE.lastFn = fnName;
    STATE.lastTarget = targetLabel(gd);

    return new Promise((resolve, reject) => {
      pending.resolvers.push({resolve, reject});
      pending.timer = setTimeout(() => {
        store.delete(key);
        flushScheduled(original, pending.ctx, pending);
      }, STATE.delayMs);
    });
  }
  ...
  function patchPlotly(){
    const P = window.Plotly;
    if (!P || P.__setaRenderDebouncePatched) return false;

    ["newPlot","react"].forEach(fnName => {
      const original = P[fnName];
      if (typeof original !== "function" || original.__setaRenderDebounceWrapped) return;

      function wrappedRenderDebounce(){
        if (!shouldDebounce()){
          return original.apply(this, arguments);
        }
        return schedulePlot(original, this, arguments, fnName);
      }

      wrappedRenderDebounce.__setaRenderDebounceWrapped = true;
      P[fnName] = wrappedRenderDebounce;
    });

    P.__setaRenderDebouncePatched = true;
    return true;
  }

  function boot(){
    ["pointerdown","mousedown","click","keydown","input","change"].forEach(evt => {
      document.addEventListener(evt, ev => {
        if (isDashboardControlTarget(ev.target)) markControlActivity();
      }, true);
    });
