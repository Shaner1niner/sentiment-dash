...
// BEGIN phase_seta_macd_histogram_width_v2
(function phase_seta_macd_histogram_width_v2(){
  if(window.__phase_seta_macd_histogram_width_v2) return;
  window.__phase_seta_macd_histogram_width_v2 = true;

  function setaHistogramBarWidthFromTraceX(x){
    try{
  ...
  function isLikelyMacdHistogramTrace(trace){
    ...
    const meta = String(trace.meta || '');
    const label = `${name} ${legendgroup} ${hover} ${text} ${meta}`;
    return /macd|hist/i.test(label);
  }

  function applySetaMacdHistogramWidth(data){
    try{
      if(!Array.isArray(data)) return data;
      data.forEach(trace => {
        if(!isLikelyMacdHistogramTrace(trace)) return;
        const width = setaHistogramBarWidthFromTraceX(trace.x);
        if(width !== undefined){
          trace.width = width;
          trace._setaMacdHistogramWidth = width;
        }
      });
    }catch(_){}
    return data;
  ...
  function patchPlotlyRenderer(){
    try{
      const P = window.Plotly;
      if(!P || P.__setaMacdHistogramWidthPatched) return !!(P && P.__setaMacdHistogramWidthPatched);
      ['newPlot','react'].forEach(fn => {
        if(typeof P[fn] !== 'function') return;
        const original = P[fn];
        P[fn] = function(gd, data, layout, config){
          applySetaMacdHistogramWidth(data);
          return original.apply(this, arguments);
        };
      });
      P.__setaMacdHistogramWidthPatched = true;
      return true;
    }catch(_){
      return false;
    }
  ...
  if(!patchPlotlyRenderer()){
    let tries = 0;
    const timer = setInterval(() => {
      tries += 1;
      if(patchPlotlyRenderer() || tries > 200) clearInterval(timer);
    }, 50);
  }

  window.setaHistogramBarWidthFromTraceX = window.setaHistogramBarWidthFromTraceX || setaHistogramBarWidthFromTraceX;
  window.applySetaMacdHistogramWidth = window.applySetaMacdHistogramWidth || applySetaMacdHistogramWidth;
})();
// END phase_seta_macd_histogram_width_v2
...
function renderReviewedBriefingPanel(panel, briefing, term, freq, rangePreset){
  const cards = briefing.briefing_cards && typeof briefing.briefing_cards === 'object' ? briefing.briefing_cards : {};
  const whatCard = cards.what_seta_sees && typeof cards.what_seta_sees === 'object' ? cards.what_seta_sees : {};
  const whyCard = cards.why_it_matters && typeof cards.why_it_matters === 'object' ? cards.why_it_matters : {};
  const evidenceCard = cards.evidence && typeof cards.evidence === 'object' ? cards.evidence : {};
  const participationCard = cards.participation_quality && typeof cards.participation_quality === 'object' ? cards.participation_quality : {};
  const evidenceSource = Array.isArray(evidenceCard.items) && evidenceCard.items.length ? evidenceCard.items : briefing.evidence;
  const evidenceItems = Array.isArray(evidenceSource)
    ? evidenceSource.map(v=>reviewedBriefingText(v)).filter(Boolean).slice(0,5)
    : [reviewedBriefingText(evidenceSource)].filter(Boolean);
  ...
  const whatCopy = reviewedBriefingText(whatCard.copy || briefing.what_seta_sees || briefing.summary || 'Reviewed briefing context is available.');
  const whyCopy = reviewedBriefingText(whyCard.copy || briefing.why_it_matters || briefing.summary || 'Context is educational and should be read with the chart evidence.');
  const trust = [reviewedBriefingText(participationCard.copy || briefing.trust_check), reviewedBriefingText(briefing.limitations), reviewedBriefingText(briefing.public_safe_disclaimer)].filter(Boolean).join(' ');
  const trustMatch = trust.match(/^([^.!?]+[.!?])\s*(.*)$/);
  const trustLead = trustMatch ? trustMatch[1] : trust;
  const trustRest = trustMatch ? trustMatch[2] : '';
  ...
  panel.hidden=false;
  panel.innerHTML=`<div class="briefingHeader briefingSemanticHeader"><div><div class="briefingTitle">${escapeHTML(briefing.headline || `SETA Briefing - ${term}`)}</div><div class="briefingMeta">${meta}${rangeNote}</div></div><div class="briefingMeta">Reviewed payload${reviewDate ? ` &middot; ${escapeHTML(reviewDate)}` : ''}</div></div><div class="briefingGrid briefingSemanticGrid"><div class="briefingCard briefingSemanticCard"><h3>What SETA Sees</h3><span class="briefingCardRole">${escapeHTML(whatCard.role || 'Interpretation')}</span><p>${escapeHTML(whatCopy)}</p></div><div class="briefingCard briefingSemanticCard"><h3>Why It Matters</h3><span class="briefingCardRole">${escapeHTML(whyCard.role || 'Implication')}</span><p>${escapeHTML(whyCopy)}</p></div><div class="briefingCard briefingSemanticCard"><h3>Evidence</h3><span class="briefingCardRole">${escapeHTML(evidenceCard.role || 'Receipts')}</span>${evidenceHtml}</div><div class="briefingCard briefingTrust briefingSemanticCard"><h3>Participation Quality</h3><span class="briefingCardRole">${escapeHTML(participationCard.role || 'Trust check')}</span><p><span class="briefingTrustLead">${escapeHTML(trustLead || 'Participation quality is unavailable for this reviewed payload.')}</span>${trustRest ? ` ${escapeHTML(trustRest)}` : ''}</p></div></div>`;
}
...
// BEGIN phase_seta_reviewed_context_compatibility_v1
(function phase_seta_reviewed_context_compatibility_v1(){
  if(window.__phase_seta_reviewed_context_compatibility_v1) return;
  window.__phase_seta_reviewed_context_compatibility_v1 = true;
  ...
  function itemPrimaryText(item){
    if(!item || typeof item !== "object") return "";
    const cards = item.briefing_cards || {};
    const what = cards.what_seta_sees || {};
    const why = cards.why_it_matters || {};
    return [
      item.title,
      item.headline,
      item.primary_read,
  ...
  function rowFieldText(row, fieldHints){
    if(!row || typeof row !== "object") return "";
    const out = [];
    Object.keys(row).forEach(function(key){
      const lk = String(key).toLowerCase();
      if(fieldHints.some(function(hint){ return lk.indexOf(hint) >= 0; })){
        const value = row[key];
  ...
  function familyFromText(text){
    ...
    const activeBearish =
      (/\bbearish pressure\b/.test(t) && !/\blatest confirmed bearish pressure\b/.test(t)) ||
      /\boverlap[_\s-]*state[^|;:]*bearish pressure/.test(t) ||
      /\bcontextual overlap[^|;:]*bearish pressure/.test(t);

    const activeBullish =
      (/\bbullish pressure\b/.test(t) && !/\blatest confirmed bullish pressure\b/.test(t)) ||
      /\boverlap[_\s-]*state[^|;:]*bullish pressure/.test(t) ||
      /\bcontextual overlap[^|;:]*bullish pressure/.test(t);

    if(activeBearish) return "bearish_pressure";
    if(activeBullish) return "bullish_pressure";

    if(/\bcompression\b/.test(t) || /\btransition[_\s/-]*risk\b/.test(t) || /\btransition\b/.test(t)){
      return "compression_transition";
    }
    if(/\brange return\b/.test(t) || /\brotated back\b/.test(t) || /\bbriefly moved outside\b/.test(t)){
      return "range_return";
    }
    if(/\binside expected range\b/.test(t) || /\bno active inside[_\s-]*zone confirmation\b/.test(t)){
      return "inside_expected";
    }
    if(/\bweakening sentiment\b/.test(t) || /\bprice resilience\b/.test(t)){
      return "price_resilience";
    }
    return "";
  }

  function activeRowFamily(row){
    if(!row || typeof row !== "object") return "";

    const overlapText = rowFieldText(row, [
      "overlap_state",
      "contextual_overlap",
      "boll_overlap_state",
      "overlap status",
    ...
    const overlapFamily = familyFromText(overlapText);
    if(overlapFamily) return overlapFamily;

    const summaryText = rowFieldText(row, [
      "dashboard_summary",
      "summary_label",
      "combined_summary",
      "ribbon",
    ...
    return familyFromText(summaryText);
  }

  function compatibleFamily(itemFamily, rowFamily){
    if(!itemFamily || !rowFamily) return true;
    if(itemFamily === rowFamily) return true;

    const compatibleGroups = [
      ["compression_transition", "inside_expected", "range_return"],
      ["price_resilience", "range_return"]
    ];

    return compatibleGroups.some(function(group){
      return group.indexOf(itemFamily) >= 0 && group.indexOf(rowFamily) >= 0;
    });
  }

  function compatibleReviewedContext(item, term, freq, rangePreset, row){
    if(!item || typeof item !== "object") return false;
    ...
    const itemFamily = familyFromText(itemPrimaryText(item));
    const rowFamily = activeRowFamily(row);

    return compatibleFamily(itemFamily, rowFamily);
  }

  function tagCompatibleFallback(item, requestedRange, row){
    try{
      if(!item || typeof item !== "object") return item;
      item.reviewed_range_fallback = true;
      item.reviewed_context_compatible = true;
      item.reviewed_requested_range = requestedRange;
      item.reviewed_active_family = activeRowFamily(row);
      item.reviewed_source_family = familyFromText(itemPrimaryText(item));
    }catch(_){}
    return item;
  }
  ...
  function wrapReviewedBriefingFor(){
    ...
    if(original.__setaReviewedContextCompatibilityWrapped) return true;
    ...
    const wrapped = function(term, freq, rangePreset, row){
      ...
      if(compatibleReviewedContext(item, term, freq, rangePreset, row)){
        try{
          console.debug("[SETA reviewed compatibility] accepted compatible reviewed fallback", {
            ...
            candidate:{
              ...
              source_family:familyFromText(itemPrimaryText(item)),
              active_family:activeRowFamily(row),
              payload_key:item && item.payload_key
            }
          });
        }catch(_){}
        return tagCompatibleFallback(item, rangePreset, row);
      }

      try{
        console.debug("[SETA reviewed compatibility] skipped incompatible reviewed fallback for active context", {
          requested:{term, freq, rangePreset},
          candidate:{
            asset:candidateAsset(item),
            frequency:candidateFrequency(item),
    ...
    wrapped.__setaReviewedContextCompatibilityWrapped = true;
  ...
  window.SETA_REVIEWED_CONTEXT_COMPATIBILITY = {
    exactReviewedContext,
    compatibleReviewedContext,
    activeRowFamily,
    wrapReviewedBriefingFor
  };
})();
// END phase_seta_reviewed_context_compatibility_v1
