...
// BEGIN phase_seta_dropdown_ui_responsiveness_v1
(function phase_seta_dropdown_ui_responsiveness_v1(){
  if (window.__phase_seta_dropdown_ui_responsiveness_v1) return;
  window.__phase_seta_dropdown_ui_responsiveness_v1 = true;

  const state = {
    enabled:true,
    pointerOpenCount:0,
    selectionPulseCount:0,
    lastOpenAt:null,
    lastSelectionAt:null
  };
  window.SETA_DROPDOWN_UI_RESPONSIVENESS = state;

  function injectStyle(){
    if (document.getElementById("phase_seta_dropdown_ui_responsiveness_v1_style")) return;
    if (!document.head) return;
    const style = document.createElement("style");
    style.id = "phase_seta_dropdown_ui_responsiveness_v1_style";
    style.textContent = `
      ...
      .setaCustomSelectWrap.open .setaCustomSelectMenu{
        animation:setaDropdownPopIn .075s ease-out;
      }
      .setaCustomSelectWrap.setaDropdownUpdating .setaCustomSelectButton{
        border-color:rgba(94,234,212,.74)!important;
        box-shadow:0 0 0 2px rgba(94,234,212,.10);
      }
      @keyframes setaDropdownPopIn{
        from{opacity:.72;transform:translateY(-2px);}
        to{opacity:1;transform:translateY(0);}
      }
      @media (prefers-reduced-motion: reduce){
        .setaCustomSelectWrap.open .setaCustomSelectMenu{animation:none;}
      }
    `;
    document.head.appendChild(style);
  }

  function closeOtherMenus(activeWrap){
    document.querySelectorAll(".setaCustomSelectWrap.open").forEach(wrap => {
  ...
  function openImmediately(btn){
    const wrap = btn && btn.closest ? btn.closest(".setaCustomSelectWrap") : null;
    if (!wrap || wrap.classList.contains("open")) return false;
    closeOtherMenus(wrap);
    wrap.classList.add("open");
    btn.setAttribute("aria-expanded", "true");
    wrap.dataset.setaPointerOpened = "1";
    state.pointerOpenCount += 1;
    state.lastOpenAt = new Date();
  ...
  function pulseSelection(target){
    const opt = target && target.closest ? target.closest(".setaCustomSelectOption") : null;
    if (!opt || opt.disabled) return;
    const wrap = opt.closest(".setaCustomSelectWrap");
    if (!wrap) return;
    wrap.classList.add("setaDropdownUpdating");
    state.selectionPulseCount += 1;
    state.lastSelectionAt = new Date();
    setTimeout(() => wrap.classList.remove("setaDropdownUpdating"), 180);
  }
  ...
  function boot(){
    ...
    document.addEventListener("pointerdown", ev => {
      const btn = ev.target && ev.target.closest ? ev.target.closest(".setaCustomSelectButton") : null;
      if (!btn) return;
      if (openImmediately(btn)) {
        // The existing click handler will fire next. We suppress just that click
        // so the menu does not immediately toggle closed again.
        ev.preventDefault();
      }
    }, true);
    ...
    document.addEventListener("click", ev => {
      const btn = ev.target && ev.target.closest ? ev.target.closest(".setaCustomSelectButton") : null;
      if (!btn) return;
      const wrap = btn.closest(".setaCustomSelectWrap");
      if (wrap && wrap.dataset.setaPointerOpened === "1") {
        delete wrap.dataset.setaPointerOpened;
        ev.preventDefault();
...
// END phase_seta_dropdown_ui_responsiveness_v1
...
(function phase_seta_custom_dropdowns_v1(){
  ...
  function optionSignature(sel){
    try{
      return Array.from(sel.children).map(child => {
        if (child.tagName === "OPTGROUP"){
          return "G:" + (child.label || "") + ":" + Array.from(child.children).map(opt => {
            return [opt.value, opt.textContent || opt.label || "", opt.disabled ? "1" : "0"].join("~");
          }).join(",");
        }
        if (child.tagName === "OPTION"){
          return "O:" + [child.value, child.textContent || child.label || "", child.disabled ? "1" : "0"].join("~");
        }
        return "";
      }).join("|");
    }catch(_){
  ...
  function refreshMenuSelection(sel, menu){
    const current = String(sel.value);
    menu.querySelectorAll(".setaCustomSelectOption").forEach(btn => {
      const selected = String(btn.dataset.value) === current;
  ...
  function renderMenuOptions(sel, menu){
    const sig = optionSignature(sel);

    // Fast path: options are unchanged, so just refresh selected state.
    if (menu.dataset.setaOptionsSignature === sig && menu.childElementCount){
      refreshMenuSelection(sel, menu);
      return;
    }

    menu.dataset.setaOptionsSignature = sig;
    menu.innerHTML = "";
    ...
    Array.from(sel.children).forEach(child => {
      if (child.tagName === "OPTGROUP"){
        ...
        Array.from(child.children).forEach(opt => frag.appendChild(optionButton(sel, opt)));
      } else if (child.tagName === "OPTION") {
        frag.appendChild(optionButton(sel, child));
      }
    });
