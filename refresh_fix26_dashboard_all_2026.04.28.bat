...
REM   7. Repairs and validates protected Evidence Handoff mounts before git staging
...
REM   - Repairs Evidence Handoff mounts after generated HTML is produced
REM   - Fails before commit/push if Evidence health or refresh integrity checks fail
REM =============================================================
...
REM Evidence Handoff protected mount guardrails.
set "EVIDENCE_MOUNT_REPAIR_HELPER=%WEBSITE_REPO%\scripts\ensure_evidence_mounts.py"
set "EVIDENCE_HEALTH_HELPER=%WEBSITE_REPO%\scripts\check_evidence_refresh_health.py"
set "REFRESH_INTEGRITY_HELPER=%WEBSITE_REPO%\scripts\check_refresh_integrity.py"
...
if not exist "%EVIDENCE_MOUNT_REPAIR_HELPER%" (
  echo [ERROR] Evidence mount repair helper not found:
  echo         %EVIDENCE_MOUNT_REPAIR_HELPER%
  goto :fail
)

if not exist "%EVIDENCE_HEALTH_HELPER%" (
  echo [ERROR] Evidence refresh health helper not found:
  echo         %EVIDENCE_HEALTH_HELPER%
  goto :fail
)

if not exist "%REFRESH_INTEGRITY_HELPER%" (
  echo [ERROR] Refresh integrity helper not found:
  echo         %REFRESH_INTEGRITY_HELPER%
  goto :fail
)
...
echo.
echo ============================================================
echo [1/9] Running chart-history exporter...
echo ============================================================
...
echo.
echo ============================================================
echo [2/9] Building SETA market screener, indicator matrix, and archetypes...
echo ============================================================
...
echo.
echo ============================================================
echo [3/9] Building Fix 26 screener JSON payload...
echo ============================================================
...
echo.
echo ============================================================
echo [4/9] Building Fix 26 public/member chart JSON payloads...
echo ============================================================
...
echo.
echo ============================================================
echo [5/9] Running dashboard smoke test...
echo ============================================================
pushd "%WEBSITE_REPO%"
...
echo.
echo ============================================================
echo [6/9] Repairing and validating Evidence Handoff mounts...
echo ============================================================
pushd "%WEBSITE_REPO%"
if errorlevel 1 (
  echo [ERROR] Could not enter website repo folder for Evidence checks.
  goto :fail
)

echo [PRE-COMMIT] Repairing Evidence Handoff mounts after generated dashboard refresh...
"%PYTHON_EXE%" "%EVIDENCE_MOUNT_REPAIR_HELPER%" --root "%WEBSITE_REPO%"
if errorlevel 1 (
  popd
  echo [ERROR] Evidence mount repair failed.
  goto :fail
)

echo [PRE-COMMIT] Validating Evidence Handoff refresh health...
"%PYTHON_EXE%" "%EVIDENCE_HEALTH_HELPER%" --root "%WEBSITE_REPO%" --no-write
if errorlevel 1 (
  popd
  echo [ERROR] Evidence refresh health check failed.
  goto :fail
)

echo [PRE-COMMIT] Validating refresh integrity...
"%PYTHON_EXE%" "%REFRESH_INTEGRITY_HELPER%" --root "%WEBSITE_REPO%" --report-only
if errorlevel 1 (
  popd
  echo [ERROR] Refresh integrity check failed.
  goto :fail
)
...
echo.
echo ============================================================
echo [7/9] Staging website repo changes...
echo ============================================================
pushd "%WEBSITE_REPO%"
if errorlevel 1 (
...
if exist refresh_fix26_dashboard_all_2026.04.28.bat git add refresh_fix26_dashboard_all_2026.04.28.bat
...
if exist scripts\ensure_evidence_mounts.py git add scripts\ensure_evidence_mounts.py
if exist scripts\check_evidence_refresh_health.py git add scripts\check_evidence_refresh_health.py
if exist scripts\check_refresh_integrity.py git add scripts\check_refresh_integrity.py
...
if "%AUTO_COMMIT%"=="1" (
  ...
  if errorlevel 2 (
    popd
    echo [ERROR] Could not inspect staged git diff before commit.
    goto :fail
  )
  ...
  if errorlevel 1 (
    ...
    if "%AUTO_PUSH%"=="1" (
      echo.
      echo ============================================================
      echo [9/9] Pushing to origin main...
      echo ============================================================
      ...
      echo.
      echo [INFO] AUTO_PUSH is 0. Commit created locally; push skipped.
    )
  ) else (
    echo.
    echo [INFO] No staged changes detected; skipping commit and push.
  )
) else (
  echo.
  echo [INFO] AUTO_COMMIT is 0. Changes are staged only.
)
...
echo   %MEMBER_JSON%
echo.
echo Evidence Handoff mount repair and integrity checks completed before git staging.
