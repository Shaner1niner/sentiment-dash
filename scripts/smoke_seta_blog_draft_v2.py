...
CONTENT_SMOKE_DIR = ROOT / "reply_agent" / "content_packets" / "_smoke_blog_draft_v2"
SNIPPETS_SMOKE_DIR = ROOT / "reply_agent" / "website_snippets" / "_smoke_blog_draft_v2"
OUTLINE_SMOKE_DIR = ROOT / "reply_agent" / "blog_outlines" / "_smoke_blog_draft_v2"
DRAFT_SMOKE_DIR = ROOT / "reply_agent" / "blog_drafts" / "_smoke_v2"
...
def repeated_phrase_count(md: str, phrase: str) -> int:
    return md.lower().count(phrase.lower())


def main() -> int:
    print("=" * 76)
    print("SETA blog draft v2 smoke test")
    print("=" * 76)
    ...
    if payload.get("schema_version") != "seta_blog_draft_v2":
    ...
    for phrase in ["Thesis", "Market context", "Synthesis", "Closing read", "Draft-only"]:
        if phrase not in md:
    ...
    if repeated_phrase_count(md, "is showing broader participation") > 4:
        fail("repetition too high: 'is showing broader participation'")
    if repeated_phrase_count(md, "the open question is whether that breadth can hold") > 3:
        fail("repetition too high: repeated open-question sentence")
    if repeated_phrase_count(md, "not a prediction or trade signal") > 3:
        fail("safety boilerplate repeated too often")
