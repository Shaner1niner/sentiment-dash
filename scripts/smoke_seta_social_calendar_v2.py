...
CONTENT_SMOKE_DIR = ROOT / "reply_agent" / "content_packets" / "_smoke_social_calendar_v2"
SNIPPETS_SMOKE_DIR = ROOT / "reply_agent" / "website_snippets" / "_smoke_social_calendar_v2"
OUTLINE_SMOKE_DIR = ROOT / "reply_agent" / "blog_outlines" / "_smoke_social_calendar_v2"
DRAFT_SMOKE_DIR = ROOT / "reply_agent" / "blog_drafts" / "_smoke_social_calendar_v2"
CALENDAR_SMOKE_DIR = ROOT / "reply_agent" / "social_calendar" / "_smoke_v2"
...
def main() -> int:
    print("=" * 76)
    print("SETA social content calendar v2 smoke test")
    print("=" * 76)
    ...
    if "```text" not in md:
        fail("Markdown should use code fences for draft posts")
    if md.count("Status: pending") < 3:
        fail("Markdown missing status lines")

    ok(f"rows generated: {len(rows)}")
    ok(f"platforms: {sorted(platforms)}")
    ok("draft-only and no-posting invariants hold")
    ok("platform voice and length checks pass")
    print("=" * 76)
    print("PASSED")
    return 0
